...
export default function NavBar() {
  ...
  return (
    ...
    <nav id="nav" className={`origin-bottom-left py-1 px-1 flex gap-1 items-center rounded-full border backdrop-blur-2xl border-violet-500/30  dark:border-violet-500/40 bg-black/5 dark:bg-white/10 transition-all duration-1000`}>
      ...
      <div id="nav-selector" className="absolute w-24 h-[80%] rounded-full bg-black/10 dark:bg-white/10 transition-all duration-100 z-[-1]" style={{ left: `${selectorPosition}px` }}></div>
      <div id="nav-selected" className="absolute w-24 h-[80%] rounded-full bg-black/20 dark:bg-white/20 transition-all duration-300 z-[-2]" style={{ left: `${backgroundPosition}px`, height: `${backgroundHeight}%`, width: `${backgroundWidth}px` }}></div>
    </nav>
