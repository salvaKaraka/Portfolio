...
export default function ActiveLink({ svg, href, label}:ActiveLinkProps) {
    ...
    return (
        <Link href={href} className={`relative flex justify-center items-center gap-[3px] hover:bg-black/10 dark:hover:bg-white/20 rounded-full py-1 px-4 transition ${ path === href || path.startsWith(`${href}/`) ? "bg-black/10 dark:bg-white/20" : ""}`}>
              {svg}
              {label}
